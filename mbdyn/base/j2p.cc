...
Elem2Param::Elem2Param(unsigned int uL, const DofOwner* pDO, flag fOut)
     :ScalarNode(uL, pDO, OutputHandler::PARAMETERS, fOut), ParameterNode(uL, pDO, 0., fOut), pElem(0), iNum(0) 
{
	NO_OP;
}
...
StrainGageParam::StrainGageParam(unsigned int uL, const DofOwner* pDO,
     ...
     :ScalarNode(uL, pDO, OutputHandler::PARAMETERS, fOut), Elem2Param(uL, pDO, fOut), dY(dy), dZ(dz) 
{
	NO_OP;
}
