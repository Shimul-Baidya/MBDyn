...
class TestAirframe(unittest.TestCase):
    def setUp(self):
        base_ref = 'global'
        self.reference_system = r.ReferenceSystem(
            label='AIRFRAME_1',
        ...
        self.airframe = r.Airframe(reference_system=self.reference_system)

    def test_initialization(self):
        """Test that the Airframe component is initialized correctly."""
        self.assertIsInstance(self.airframe, r.Airframe)
        self.assertEqual(self.airframe.reference_system.label, 'AIRFRAME_1')
    ...
    def test_create_nodes(self):
        ...
        self.assertEqual(node.idx, 10000) # Using the hardcoded value for now
        self.assertEqual(node.position, self.airframe.reference_system.position_wrt_base)
        self.assertEqual(node.orientation, self.airframe.reference_system.orientation_wrt_base)
        self.assertEqual(node.velocity, self.airframe.reference_system.velocity_wrt_base)
        self.assertEqual(node.angular_velocity, self.airframe.reference_system.angular_velocity_wrt_base)
